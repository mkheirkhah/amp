...
//                    MakeUintegerChecker<uint32_t>(1))
    ...
    .AddAttribute("OutputFileName",
                  ...
                  MakeStringChecker())
    .AddAttribute("OutputFileNameDctcp",
                  "Output file name for recording dctcp's fraction of marked pkts",
                   StringValue("NULL"),
                   MakeStringAccessor(&MpTcpBulkSendApplication::m_outputFileNameDctcp),
                   MakeStringChecker())
    ...
    .AddAttribute ("DCTCP",
                   ...
                   BooleanValue (false),
                   MakeBooleanAccessor (&MpTcpBulkSendApplication::m_DCTCP),
                   MakeBooleanChecker ())
    .AddAttribute ("FlowLayer",
                   "Indicating the layer of the network topology that a flow should pass",
                   StringValue("NULL"),
                   MakeStringAccessor(&MpTcpBulkSendApplication::m_flowLayer),
                   MakeStringChecker())
    .AddAttribute ("SocketModel",
                   "Type of Socket, e.g. MPTCP, TCP, PS, MMPTCP, DCMMPTCP, DCMPTCP, DCPS, DCTCP ",
                   StringValue("NULL"),
                   MakeStringAccessor(&MpTcpBulkSendApplication::m_socketModel),
                   MakeStringChecker())
    .AddAttribute ("CongestionControl",
                   "Congestion Control Algorithm for MPTCP",
                   StringValue("RTT_Compensator"),
                   MakeStringAccessor(&MpTcpBulkSendApplication::m_cc),
                   MakeStringChecker())
    .AddAttribute ("SlowDownEcnLike",
                   "Slow down sending rate based on ECN",
                   BooleanValue (false),
                   MakeBooleanAccessor (&MpTcpBulkSendApplication::m_slowDownEcnLike),
                   MakeBooleanChecker ())
    .AddAttribute ("DctcpFastAlpha",
                   "DCTCP Non-Smoothed Alpha",
                   BooleanValue (false),
                   MakeBooleanAccessor (&MpTcpBulkSendApplication::m_dctcpFastAlpha),
                   MakeBooleanChecker ())
  ...
  if (!m_socket)
    {
      ...
      m_socket->SetFlowLayer(m_flowLayer);
      m_socket->SetSocketModel(m_socketModel);
      ...
      if (result == 0)
        {
          ...
          m_socket->SetFlowCompletedCallback (MakeCallback (&MpTcpBulkSendApplication::NotifyFCT, this));
//        m_socket->SetCloseCallbacks(MakeCallback (&MpTcpBulkSendApplication::NormalClose, this),
//                                    MakeCallback (&MpTcpBulkSendApplication::ErrorClose, this) );
...
void
MpTcpBulkSendApplication::NotifyFCT(Ptr<Socket> socket)
{
  NS_LOG_FUNCTION(this << socket);
  if (!m_Notify.IsNull ())
    {
      m_Notify (m_node, m_index, m_totBytes, 0, 0);
    }
}
...
/*
void
MpTcpBulkSendApplication::NormalClose (Ptr<Socket> s)
{
	cout << "MpTcpBulkSendApplication::NormalClose" << endl;
  s->Close();
	m_connected = false;
  DoubleValue dv1, dv2;
  s->GetAttribute("InstantRate", dv1);
  s->GetAttribute("TimeTag", dv2);
  s = 0;
  if (!m_Notify.IsNull() )
    m_Notify(m_node, m_index, m_totBytes, dv2.Get(), dv1.Get() );
}

void
MpTcpBulkSendApplication::ErrorClose (Ptr<Socket> s)
{
  m_connected = false;
}
*/
