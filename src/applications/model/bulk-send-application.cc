    ...
    .AddAttribute ("MaxBytes",
                   ...
                   MakeUintegerChecker<uint32_t> ())
                       ...
                       .AddAttribute("FlowType",
                                     ...
                                     StringValue("Large"),
                                     MakeStringAccessor(&BulkSendApplication::m_flowType),
                                     MakeStringChecker())
                       .AddAttribute("OutputFileName",
                                     "Output file name",
                                     StringValue("NULL"),
                                     MakeStringAccessor(&BulkSendApplication::m_outputFileName),
                                     MakeStringChecker())
  ...
  if (!m_socket)
    {
      ...
      if (Inet6SocketAddress::IsMatchingType (m_peer))
        ...
        {
          m_socket->Bind ();
          DynamicCast<TcpSocketBase>(m_socket)->SetFType(m_flowType);
          DynamicCast<TcpSocketBase>(m_socket)->SetOFName(m_outputFileName);
        }

      m_socket->Connect (m_peer);
      DynamicCast<TcpSocketBase>(m_socket)->SetFId(m_flowId);
