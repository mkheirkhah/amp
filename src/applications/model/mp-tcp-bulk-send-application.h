...
typedef Callback<void, Ptr<Node>, uint32_t, uint32_t, double, double> BulkSendAppNotify_t;
...
// uint32_t       m_bufferSize;   // Application buffer size
  ...
  std::string     m_outputFileNameDctcp;
  bool            m_DCTCP;
  string          m_flowLayer;
  string          m_socketModel;
  string          m_cc;
...
public:
//void NormalClose (Ptr<Socket> s);
//void ErrorClose  (Ptr<Socket> s);
  void NotifyFCT   (Ptr<Socket> s);
  BulkSendAppNotify_t m_Notify;
  uint32_t            m_index;
};
