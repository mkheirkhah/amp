...
RttHistory::RttHistory (SequenceNumber32 s, uint32_t c, Time t, uint64_t marked, uint64_t unmarked)
  : seq (s), count (c), time (t),
  nonMarked (unmarked), marked (marked), retx (false)
{
  NS_LOG_FUNCTION (this);
}

RttHistory::RttHistory (const RttHistory& h)
  : seq (h.seq), count (h.count), time (h.time),
   nonMarked (h.nonMarked), marked (h.marked), retx (h.retx)
{
  NS_LOG_FUNCTION (this);
}
...
RttEstimator::RttEstimator ()
  : m_next (1), m_history (),
    m_nSamples (0),
    m_multiplier (1),
	m_g(0), m_marked(0), m_nonMarked(0), m_alpha(0)
{ 
	m_delta_marked = 0;
	m_delta_unmarked = 0;
	m_fracMarkPkt = 0;
  ...
  while(m_history.size () > 0)
    {
      ...
      m_delta_marked = delta_marked; // Just for debugging
      m_delta_unmarked = delta_unmarked;  // Just for debugging
      f = delta_marked ? delta_marked / (delta_unmarked + delta_marked) : 0;
      m_fracMarkPkt = f;	 // Just for debugging
