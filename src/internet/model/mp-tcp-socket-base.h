...
public: // public methods
  ...
  void SetFlowSize(uint32_t);
  void SetFlowId(uint32_t);
  void SetFlowType(string);
  void SetFlowLayer(string);
  void SetSocketModel(string);
  void SetOutputFileName(string);
  void SetOutputFileNameDctcp(string);
  virtual void SetDupAckThresh(uint32_t);
  void SetDctcp(bool);
  bool GetDctcp();
  void SetSlowDownEcnLike(bool);
  void SetDctcpFastAlpha(bool);
...
public: // public variables
  ...
  string outputFileNameDctcp;
  double goodput;
  bool m_largePlotting;
  bool m_shortPlotting;
  bool m_ratePlotSf;
  bool m_ratePlotCl;
  uint32_t m_totalSentBytes;

  double m_rateInterval;
  //uint64_t lastNextTxSequence;
  vector<pair<double, double> > rateTracerCl;
  EventId m_nextRateEvent;
  bool m_alphaPerAck;
  uint32_t m_rGap;
  bool m_shortFlowTCP;
  bool m_backgroundFlow;
  //
  GnuplotCollection gnu;
  std::list<uint32_t> sampleList;
  bool hasSampleListDone;
...
protected: // protected methods
  ...
  void calculateFastAlpha(uint8_t sFlowIdx);
  ...
  std::string PlotFileNameGenerator(string s);
  void GenerateCWNDPlot();
  void GenerateSendvsACK();
  void GenerateRTT();
  void GenerateCwnd();
  void GenerateTotalCwnd();
  void GenerateAlpha();
  void GenerateCwndTracer();
  void GeneratePktCount();
  void GeneratePlots();
  void GenerateRateFile();
  void GenerateRate();
  void GenerateRateTracerSf();
  void GenerateRateTracerCl();
  void IsLastAck();
  virtual void GeneratePlotsOutput();
  virtual string GenerateEpsFileName ();
  virtual void DoGenerateOutPutFile();
  void DoDoGenerateOutPutFile(TypeId tid); // NON-Virtual
  virtual string GetTypeIdName();
  string TcpFlagPrinter(uint8_t);
  string IsActive(uint32_t);
...
//  virtual void AddContPktTag(Ptr<Packet> p);
  void AddEcmpTag(Ptr<Packet> p, uint8_t sFlowIdx);
  void GenerateDctcpAlpha();    //DCTCP Debugging
...
protected: // protected variables
  ...
  bool m_slowDownXmpLike;
  bool m_slowDownEcnLike;
  bool m_dctcpFastAlpha;
  bool m_ADCT;
  bool m_ADCTcontrol;
  double m_ADCTg;
  ...
  bool              m_ecn;
  uint32_t          m_rwndScale;
  uint8_t           m_initialRand;
  bool              m_disjoinPath;
