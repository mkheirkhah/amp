      ...
      if ((opt->optName == OPT_MPC) && hasSyn)
        ...
        { // In this case there should be endPoint with this token, so look for a match on all endpoints.
          ...
          if (it != m_TokenMap.end())
            {
              //NS_LOG_UNCOND("TcpL4Protocol::Receive -> OPT_JOIN -> Token " << Token << " has find forwardup to it");
              if ((*it).second != NULL)
                {
                ...
                {
                  return IpL4Protocol::RX_ENDPOINT_CLOSED; // TODO need to be tested
                  NS_LOG_UNCOND("TcpL4Protocol -> MPTCP with Join option and endpoint is NULL");
                }
            }
        }
      else if (opt->optName == OPT_DSN)
        {
          ...
          //NS_LOG_UNCOND("TcpL4Protocol received DSS! pScatter: " << (int)pScatter);
          if (pScatter > 0)
            {
              Token = ((OptDataSeqMapping*) opt)->receiverToken;
              if (Token == 0)
                {
                  NS_LOG_UNCOND("*** ["<< m_node->GetId() << "] Token: " << Token << " pScatter: " << (int)pScatter);
              ...
              if (it != m_TokenMap.end())
                {
                  //NS_LOG_UNCOND("TcpL4Protocol::Receive -> OPT_DSN -> It is packetScatter and its token " << Token);
                  if ((*it).second != NULL)
                    {
                      ((*it).second)->ForwardUp(packet, ipHeader, srcPort, incomingInterface);
                      return IpL4Protocol::RX_OK;
                    ...
                    { // Endpoint is NULL!
                      NS_LOG_UNCOND("TcpL4Protocol -> pScatter and endpoint is NULL");
                      return IpL4Protocol::RX_ENDPOINT_CLOSED;
                    }
                }
            }
          else
            { // If packet-scatter is disabled then Token should be zero too!
              NS_ASSERT_MSG(((OptDataSeqMapping*) opt)->receiverToken == 0, "remoteToken: " << ((OptDataSeqMapping*) opt)->receiverToken << " pScatter: " << pScatter << " Header: " << tcpHeader);
            }
...
bool
TcpL4Protocol::RemoveLocalToken(uint32_t localToken)
{
  TokenMaps::iterator it;
  it = m_TokenMap.find(localToken);
  if (it != m_TokenMap.end())
    {
      NS_LOG_UNCOND("["<< m_node->GetId() << "] Token Has been erased from TokenMap -> localToken " << it->first << " -> " << it->second << " ContainerSize: " << m_TokenMap.size());
      if (it->second != 0)
        {
          NS_LOG_UNCOND("["<< m_node->GetId() << "] Its endPoint point to NULL! before erase" << m_TokenMap.size());
          it->second = NULL;
        }
      m_TokenMap.erase(it);
      NS_LOG_UNCOND("["<< m_node->GetId() << "] After erase: " << m_TokenMap.size() << " ContainerSize: " << m_TokenMap.size());
      return true;
    }
  return false;
}
