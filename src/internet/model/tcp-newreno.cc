...
TcpNewReno::NewAck (const SequenceNumber32& seq)
{
  ...
  if (m_inFastRec && seq < m_recover)
    { // Partial ACK, partial window deflation (RFC2582 sec.3 bullet #5 paragraph 3)
      ...
      PartialAck.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_cWnd));
      ssthreshtrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_ssThresh));
      pAckHit++;
    ...
    { // Full ACK (RFC2582 sec.3 bullet #5 paragraph 2, option 1)
      ...
      FullAck.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_cWnd));
      ssthreshtrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_ssThresh));
      fAck++;
    }
  ...
  if (m_cWnd < m_ssThresh)
    { // Slow start mode, add one segSize to cWnd. Default m_ssThresh is 65535. (RFC2001, sec.1)
      ...
      SSTrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_cWnd));
      ssthreshtrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_ssThresh));
    ...
    { // Congestion avoidance mode, increase by (segSize*segSize)/cwnd. (RFC2581, sec.3.1)
      ...
      //cwndTrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_cWnd));
      CATrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_cWnd));
      ssthreshtrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_ssThresh));
    }
...
TcpNewReno::DupAck (const TcpHeader& t, uint32_t count)
{
  ...
  if (count == m_retxThresh && !m_inFastRec)
    { // triple duplicate ack triggers fast retransmit (RFC2582 sec.3 bullet #1)
      ...
      DoRetransmit ();
      FastReTx.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_cWnd));
      ssthreshtrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_ssThresh));
      fReTx++;
    }
  else if (m_inFastRec)
    { // Increase cwnd for every additional dupack (RFC2582, sec.3 bullet #3)
      ...
      DupAcks.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_cWnd));
      ssthreshtrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_ssThresh));
      fRecovery++;
...
TcpNewReno::Retransmit (void)
{
  ...
  timeOuts++;
  timeOutTrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_cWnd));
  ssthreshtrack.push_back(std::make_pair(Simulator::Now().GetSeconds(), m_ssThresh));
}
