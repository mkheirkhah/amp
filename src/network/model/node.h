...
typedef struct __DatacenterJob_t
{
  uint32_t jobId;
  Time tmBegin;
  Ptr<Node> requestNode;
  std::map<uint32_t, Ptr<Node> > requestFlows; // appid, des
  uint32_t flowNum;
  double nextJobArrival; // ms
} DatacenterJob_t;
...
public: // Morteza
  uint32_t m_locked;
  std::map<uint32_t, DatacenterJob_t*> m_dcJobs; // appid, job
  std::map<uint32_t, Ptr<Node> > m_largeFlows;   // appid, desnode
};
