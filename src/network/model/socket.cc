...
void
Socket::SetFlowCompletedCallback(Callback<void, Ptr<Socket> > flowCompleted)
{ //Morteza
  m_flowComlpeted = flowCompleted;
}

void
Socket::NotifyFlowCompleted()
{
  if (!m_flowComlpeted.IsNull ())
    {
      m_flowComlpeted (this);
    }
}
void
...
Socket::DoDispose (void)
{
  ...
  m_flowComlpeted = MakeNullCallback<void,Ptr<Socket> > ();
}
